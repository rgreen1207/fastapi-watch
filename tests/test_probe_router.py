import pytest
from fastapi import FastAPI
from fastapi_watch import HealthRegistry, ProbeGroup
from fastapi_watch.probes import NoOpProbe
from fastapi_watch.models import ProbeStatus


# ---------------------------------------------------------------------------
# add()
# ---------------------------------------------------------------------------

def test_add_single_probe():
    router = ProbeGroup()
    probe = NoOpProbe(name="db")
    router.add(probe)
    assert len(router._probes) == 1
    assert router._probes[0] == (probe, True)


def test_add_non_critical():
    router = ProbeGroup()
    probe = NoOpProbe(name="cache")
    router.add(probe, critical=False)
    assert router._probes[0] == (probe, False)


def test_add_returns_self_for_chaining():
    router = ProbeGroup()
    result = router.add(NoOpProbe(name="a"))
    assert result is router


def test_add_deduplicates_by_identity():
    router = ProbeGroup()
    probe = NoOpProbe(name="a")
    router.add(probe)
    router.add(probe)
    assert len(router._probes) == 1


def test_add_allows_different_instances_same_name():
    router = ProbeGroup()
    router.add(NoOpProbe(name="x"))
    router.add(NoOpProbe(name="x"))
    assert len(router._probes) == 2


# ---------------------------------------------------------------------------
# add_probes()
# ---------------------------------------------------------------------------

def test_add_probes_adds_all():
    router = ProbeGroup()
    probes = [NoOpProbe(name="a"), NoOpProbe(name="b"), NoOpProbe(name="c")]
    router.add_probes(probes)
    assert len(router._probes) == 3


def test_add_probes_critical_flag_applied_to_all():
    router = ProbeGroup()
    probes = [NoOpProbe(name="a"), NoOpProbe(name="b")]
    router.add_probes(probes, critical=False)
    for _, critical in router._probes:
        assert critical is False


def test_add_probes_returns_self():
    router = ProbeGroup()
    result = router.add_probes([NoOpProbe(name="a")])
    assert result is router


def test_add_probes_deduplicates():
    router = ProbeGroup()
    probe = NoOpProbe(name="a")
    router.add_probes([probe, probe])
    assert len(router._probes) == 1


# ---------------------------------------------------------------------------
# include()
# ---------------------------------------------------------------------------

def test_include_merges_probes():
    sub = ProbeGroup()
    sub.add(NoOpProbe(name="a"))
    sub.add(NoOpProbe(name="b"))

    parent = ProbeGroup()
    parent.include(sub)
    assert len(parent._probes) == 2


def test_include_preserves_criticality():
    sub = ProbeGroup()
    probe_crit = NoOpProbe(name="critical-svc")
    probe_opt = NoOpProbe(name="optional-svc")
    sub.add(probe_crit, critical=True)
    sub.add(probe_opt, critical=False)

    parent = ProbeGroup()
    parent.include(sub)

    by_name = {p.name: c for p, c in parent._probes}
    assert by_name["critical-svc"] is True
    assert by_name["optional-svc"] is False


def test_include_returns_self():
    parent = ProbeGroup()
    sub = ProbeGroup()
    result = parent.include(sub)
    assert result is parent


def test_include_deduplicates_by_identity():
    probe = NoOpProbe(name="shared")
    sub = ProbeGroup()
    sub.add(probe)

    parent = ProbeGroup()
    parent.add(probe)         # already registered
    parent.include(sub)  # should not add again
    assert len(parent._probes) == 1


def test_include_empty_sub_is_noop():
    parent = ProbeGroup()
    parent.add(NoOpProbe(name="a"))
    parent.include(ProbeGroup())
    assert len(parent._probes) == 1


# ---------------------------------------------------------------------------
# Nesting
# ---------------------------------------------------------------------------

def test_nested_routers_compose_correctly():
    leaf_a = ProbeGroup()
    leaf_a.add(NoOpProbe(name="db"))

    leaf_b = ProbeGroup()
    leaf_b.add(NoOpProbe(name="cache"), critical=False)

    mid = ProbeGroup()
    mid.include(leaf_a)
    mid.include(leaf_b)

    root = ProbeGroup()
    root.include(mid)

    assert len(root._probes) == 2
    by_name = {p.name: c for p, c in root._probes}
    assert by_name["db"] is True
    assert by_name["cache"] is False


def test_chaining_add_and_include():
    sub = ProbeGroup()
    sub.add(NoOpProbe(name="sub-probe"))

    router = ProbeGroup()
    router.add(NoOpProbe(name="a")).add(NoOpProbe(name="b")).include(sub)
    assert len(router._probes) == 3


# ---------------------------------------------------------------------------
# HealthRegistry integration
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_registry_routers_param_registers_probes():
    router = ProbeGroup()
    router.add(NoOpProbe(name="from-router"))

    app = FastAPI()
    registry = HealthRegistry(app, groups=[router])
    results = await registry.run_all()
    assert any(r.name == "from-router" for r in results)


@pytest.mark.asyncio
async def test_registry_routers_param_multiple_routers():
    router_a = ProbeGroup()
    router_a.add(NoOpProbe(name="a"))

    router_b = ProbeGroup()
    router_b.add(NoOpProbe(name="b"))

    app = FastAPI()
    registry = HealthRegistry(app, groups=[router_a, router_b])
    results = await registry.run_all()
    names = {r.name for r in results}
    assert "a" in names and "b" in names


@pytest.mark.asyncio
async def test_registry_routers_param_preserves_criticality():
    router = ProbeGroup()
    router.add(NoOpProbe(name="optional"), critical=False)

    app = FastAPI()
    registry = HealthRegistry(app, groups=[router])
    probe_entry = registry._probes[0]
    assert probe_entry[1] is False  # critical flag


@pytest.mark.asyncio
async def test_include_on_registry_adds_probes():
    router = ProbeGroup()
    router.add(NoOpProbe(name="via-include"))

    app = FastAPI()
    registry = HealthRegistry(app)
    registry.include(router)

    results = await registry.run_all()
    assert any(r.name == "via-include" for r in results)


def test_include_on_registry_returns_self():
    router = ProbeGroup()
    app = FastAPI()
    registry = HealthRegistry(app)
    result = registry.include(router)
    assert result is registry


@pytest.mark.asyncio
async def test_include_on_registry_preserves_criticality():
    router = ProbeGroup()
    router.add(NoOpProbe(name="opt"), critical=False)

    app = FastAPI()
    registry = HealthRegistry(app)
    registry.include(router)

    assert registry._probes[0][1] is False


@pytest.mark.asyncio
async def test_include_on_registry_deduplicates():
    probe = NoOpProbe(name="shared")
    router = ProbeGroup()
    router.add(probe)

    app = FastAPI()
    registry = HealthRegistry(app)
    registry.add(probe)
    registry.include(router)

    assert len(registry._probes) == 1


def test_include_on_registry_is_chainable():
    router_a = ProbeGroup()
    router_b = ProbeGroup()
    app = FastAPI()
    registry = HealthRegistry(app)
    result = registry.include(router_a).include(router_b)
    assert result is registry
